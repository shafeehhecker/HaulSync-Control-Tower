...
export function StatCard({ label, value, icon: Icon, color = 'amber', delta, pulse }) {
  const c = colorMap[color] || colorMap.amber;
  return (
    <div className="card p-4 animate-fade-in">
      <div className="flex items-start justify-between mb-3">
        <p className="text-xs text-zinc-500 font-medium uppercase tracking-wider">{label}</p>
        <div className={`w-8 h-8 rounded-lg ${c.bg} border ${c.border} flex items-center justify-center ${pulse ? 'pulse-amber' : ''}`}>
...
const statusMap = {
  ...
  ACTIVE:           'badge-green',
  INACTIVE:         'badge-zinc',
  UNPLANNED_HALT:   'badge-red',
  ROUTE_DEVIATION:  'badge-orange',
  DELAY_RISK:       'badge-amber',
  SLA_BREACH_RISK:  'badge-red',
  NIGHT_DRIVING:    'badge-purple',
  GEOFENCE_VIOLATION:'badge-orange',
};
...
export function SeverityDot({ level }) {
  const map = { HIGH:'bg-red-400 pulse-red', MEDIUM:'bg-amber-400 pulse-amber', LOW:'bg-blue-400' };
  return <span className={`inline-block w-2 h-2 rounded-full ${map[level] || 'bg-zinc-500'}`} />;
}
...
export function LivePulse({ label = 'LIVE' }) {
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className="text-[10px] font-mono text-green-400 tracking-widest">{label}</span>
    </div>
  );
}

// ── Timeline entry ────────────────────────────────────────────────────────────
export function TimelineEntry({ time, title, description, color = 'amber', last }) {
  const c = { amber:'bg-amber-500', red:'bg-red-500', green:'bg-green-500', blue:'bg-blue-500', zinc:'bg-zinc-600' };
  return (
    <div className="flex gap-3">
      <div className="flex flex-col items-center">
        <div className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${c[color] || c.zinc}`} />
        {!last && <div className="w-px flex-1 bg-zinc-800 mt-1" />}
      </div>
      <div className="pb-4">
        <p className="text-[11px] text-zinc-500 font-mono">{time}</p>
        <p className="text-sm font-medium text-zinc-200 mt-0.5">{title}</p>
