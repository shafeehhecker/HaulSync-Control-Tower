...
import {
  Layers, LayoutDashboard, Radio, AlertTriangle,
  Bell, ClipboardList, BarChart2, Settings, Users,
  LogOut, Menu, X, ChevronRight, Zap,
} from 'lucide-react';
import { useAuth } from '../../context/AuthContext';
import { LivePulse } from '../common';

const NAV = [
  { to: '/',            label: 'Dashboard',       icon: LayoutDashboard, exact: true },
  { to: '/live',        label: 'Live Operations', icon: Radio },
  { to: '/exceptions',  label: 'Exceptions',      icon: AlertTriangle },
  { to: '/alerts',      label: 'Alerts',          icon: Bell },
  { to: '/action-log',  label: 'Action Log',      icon: ClipboardList },
  { to: '/analytics',   label: 'Analytics',       icon: BarChart2 },
  { to: '/rules',       label: 'Alert Rules',     icon: Settings },
  { to: '/users',       label: 'Users',           icon: Users },
];

function NavItem({ to, label, icon: Icon, exact, badge, onClick }) {
  return (
    ...
    >
      ...
      {badge ? (
        <span className="text-[10px] font-mono bg-red-500/20 text-red-400 border border-red-500/30 px-1.5 py-0.5 rounded-md">
          {badge}
        </span>
      ) : (
      ...
      <div className="flex items-center gap-3 px-4 py-5 border-b border-zinc-800/60">
        <div className="relative">
          <div className="w-8 h-8 bg-amber-500 rounded-xl flex items-center justify-center flex-shrink-0">
            <Layers size={15} className="text-zinc-950" />
          </div>
        </div>
        <div>
          <p className="font-display font-bold text-zinc-100 text-sm leading-none">HaulSync</p>
          <p className="text-[10px] text-amber-400/80 font-medium tracking-wider mt-0.5">CONTROL TOWER</p>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <div className="lg:hidden flex items-center gap-3 px-4 py-3 bg-zinc-900 border-b border-zinc-800/60">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-display font-bold text-zinc-100 text-sm">Control Tower</span>
          </div>
        </div>
